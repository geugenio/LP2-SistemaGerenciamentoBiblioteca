...
public class RealizarDevolucaoController {
    ...
    public RealizarDevolucaoController(EmprestimoService emprestimoService) {
    ...
    public void confirmar() {
        try {
            ...
            Optional<Livro> resultado = emprestimoService.devolver(idLivro, idUsuario);

            if (resultado.isPresent()) {
                mostrarAlerta("Sucesso", "Livro devolvido com sucesso!");
                fecharJanela();
            } else {
                lblMensagem.setText("Livro ou Usuário não encontrado.");
            }

        } catch (NumberFormatException e) {
            lblMensagem.setText("Os IDs devem ser números.");
